...
{
    string OPAYmerchantid = ConfigurationManager.AppSettings["OPAYmerchantid"];
    string OPAYprivatekey = ConfigurationManager.AppSettings["OPAYprivatekey"];
    ...
    public bool ActivarUsr(int idusr){
        bool res = false;
        string SaltCryptoKey = ConfigurationManager.AppSettings["SaltCryptoKey"];
        string PwdCryptoKey = ConfigurationManager.AppSettings["PwdCryptoKey"];
        string stm = "update usuario set activo = 1 where idusuario = " + idusr +";";
        if (ExecuteSQL(stm) > 0)
            res = true;
            string usuario = "select * from usuario where idusuario = " + idusr.ToString() +";";
            DataTable dtusr = SelectSQL(usuario);
            if (dtusr.Rows.Count > 0){
                Usuario u = new Usuario() { email = Crypto.DecryptAes(BackBitConv(dtusr.Rows[0]["email"].ToString()), PwdCryptoKey, Crypto.String2ByteArray(SaltCryptoKey)), nip = Crypto.DecryptAes(BackBitConv(dtusr.Rows[0]["nip"].ToString()), PwdCryptoKey, Crypto.String2ByteArray(SaltCryptoKey)) };
                EnviaMail(u.email, 'B', "", u);
            }
            
            //TODO Enviar email to L
        else
            res = false;
        return (res);
    }

    public int AltaUsrV2(MasTicket.Usuario u, int idusrreferido = 0)
    {
        int res = 0;
        Random r = new Random();
        int verif = r.Next(100000, 999999);
    ...
    private int AltaTarjeta(MasTicket.Tarjeta t, string deviceid, string customerid, out Card c)
    {
        string SaltCryptoKey = ConfigurationManager.AppSettings["SaltCryptoKey"];
        string PwdCryptoKey = ConfigurationManager.AppSettings["PwdCryptoKey"];

        OpenpayAPI openpay = new OpenpayAPI(OPAYprivatekey, OPAYmerchantid);
        openpay.Production = false;
        c = new Card();
        c.TokenId = t.permtoken;
        c.DeviceSessionId = deviceid;
        c = openpay.CardService.Create(customerid, c);
    ...
    public void recargaRegaloCat(int idusuario, string email)
    {
                            ...
                            if (b > 0) {
                                ...
                                using (var conn = new MySqlConnection(cs))
                                {
                                    ...
                                    using (var cmd = new MySqlCommand(command, conn))
                                    {
                                        ...
                                        cmd.Parameters.Add("@idusr", MySqlDbType.Int32).Value = idusuario;
                                        cmd.Parameters.Add("@monto", MySqlDbType.Decimal).Value = Decimal.Parse(datos[1]);
                                        conn.Open();
                                        try
                                        {
                                            if (cmd.ExecuteNonQuery() > 0)
    ...
    public async Task<string> AltaRecargaViaWallet(MasTicket.Recarga r)
    {
        ...
        try
        {
            File.AppendAllText("c:/Ceo/AC/Logs/syslog/ac1.txt", 
                DateTime.Now.ToString("HH:mm:ss") + " " + cs + System.Environment.NewLine);
        }
        catch (Exception ex)
        {
        }
        ...
        using (var conn = new MySqlConnection(cs))
        {
            ...
            if (dtusr.Rows.Count > 0)
            {
                ...
                using (var cmd = new MySqlCommand(command, conn))
                {
                    ...
                    cmd.Parameters.Add("@os", MySqlDbType.VarChar, 100).Value = r.os;
                    conn.Open();
                    //Write2Log("se abrio bien la conexion");
                    if (cmd.ExecuteNonQuery() > 0)
                    {
                        //Write2Log(cmd.LastInsertedId);
                        ...
                        using (var cmd2 = new MySqlCommand("CargaMonedero", conn))
                        {
                            ...
                            if (int.Parse(cmd2.ExecuteScalar().ToString()) == 0)
                            {
                                ...
                                if (err == 0)
                                {
                                    recargaRegaloCat(u.idusuario, u.email);
                                }
                                errRs = tmp.errRs;
                                tresp = tmp.tresp;
                                string stmrs = "";
    ...
    public int AltaRecarga(MasTicket.Recarga r)
    {
        int idr = 0;
        string cs = ConfigurationManager.ConnectionStrings["ac"].ConnectionString;
        DataSet ds = new DataSet();
    ...
    */
    ...
    /*    
        ...
        //VALIDAR AQUI CARRIER
            //Invalido
            try
            {
                string carrierName = "";
                //const string accountSid = "ACdaabdbe9f4986207e0b77934d7e26864";
                //const string authToken = "c053876591a03327e06c865c19878468";
                const string accountSid = "ACfa28408503403889e7315789bd94f012";
                const string authToken = "c0f6df13069406883584f7d7edc75486";
                var lookupsClient = new LookupsClient(accountSid, authToken);
                var numero_final = "52" + r.numerorecarga;
                var phoneNumber = lookupsClient.GetPhoneNumber(numero_final, true);

                string errorCode = "";
                if (phoneNumber.RestException != null)
                {
                    errorCode = phoneNumber.RestException.Code;
                }

                if (errorCode == "20404")
                {
                    e = new errRecarga { idtarjeta = idnuevatarjeta, PaymentID = paymentid, err = 4, errVs = 11, errRs = errRs, tresp = new tresp { rcode_description = "El número ingresado no existe." } };
                    return (JsonConvert.SerializeObject(e));
                }

                var codigo_numero = 0;
                if (phoneNumber.Carrier.Type == "landline")
                {
                    codigo_numero = 0;
                }

                var nombre = phoneNumber.Carrier.Name.ToLower();

                if (nombre.Contains("unefon"))
                {
                    codigo_numero = 5;
                }
                if (nombre.Contains("nextel")) //
                {
                    codigo_numero = 0;
                }
                if (nombre.Contains("iusacell"))
                {
                    codigo_numero = 4;
                }

                if (nombre.Contains("telcel"))
                {
                    codigo_numero = 1;
                    //Alo 903
                }
                if (nombre.Contains("movistar"))
                {
                    codigo_numero = 2;
                    //Virgin 30 Cierto 900 Maztiempo 902
                }

                carrierName = phoneNumber.Carrier.Name;
                if (r.idoperadora != codigo_numero)
                {
                    if ((r.idoperadora == 4 || r.idoperadora == 5) && (codigo_numero == 4 || codigo_numero == 5))//Iusacell o Unefon
                    {
                        //Iusacell o Unefon
                        carrierName = "Iusacell o Unefon";
                    }
                    else if ((r.idoperadora == 30 || r.idoperadora == 900 || r.idoperadora == 901 || r.idoperadora == 902) && codigo_numero == 2) //Virgin, Cierto, Tuenti, Maztiempo son Posible movistar
                    {

                        //Movistar OK
                        carrierName = "Movistar (Virgin, Cierto, Tuenti, Maztiempo)";

                    }
                    else if (r.idoperadora == 903 && codigo_numero == 1) //ALO es posible Telcel
                    {
                        //Telcel OK
                        carrierName = "Telcel (ALO)";
                    }
                    else
                    {
                        e = new errRecarga { idtarjeta = idnuevatarjeta, PaymentID = paymentid, err = 4, errVs = 11, errRs = errRs, tresp = new tresp { rcode_description = "Número no corresponde a la compañía seleccionada, por favor verifique." } };
                        return (JsonConvert.SerializeObject(e));
                        //return "El carrier ingresado" + idoperadora + " no corresponde al Carrier seleccionado. " + nombre;
                    }
                }            
            ...
            {
                //Aqui comento Pablo 08032018
                e = new errRecarga { idtarjeta = idnuevatarjeta, PaymentID = paymentid, err = 4, errVs = 11, errRs = errRs, tresp = new tresp { rcode_description = "Ocurrio un error en la validación del número. Ex"+ exc.Message } };
                return (JsonConvert.SerializeObject(e));
            }
    ...
    public async Task<string> Carga3DSecure(Recarga r, RecargaMonedero rm, string deviceid, Tarjeta t, int vez)
    {
        Response3dSecure resp = new Response3dSecure() { err = false, desc = "" };
        int err = -1; int errVs = -1; int errRs = -1; string paymentid = "";
        int idnuevatarjeta = 0; int idnuevarecarga = 0;
        errRecarga e = new errRecarga();
        #region VALIDAR AQUI CARRIER
        //Invalido
        try
        {
            string carrierName = "";
            ...
            if (r.idoperadora != codigo_numero)
            {
                ...
                {
                    //e = new errRecarga { idtarjeta = idnuevatarjeta, PaymentID = paymentid, err = 4, errVs = 11, errRs = errRs, tresp = new tresp { rcode_description = "Número no corresponde a la compañía seleccionada, por favor verifique." } };
                    resp = new Response3dSecure { err = true, desc = "Número no corresponde a la compañía seleccionada, por favor verifique." };
                    //return "El carrier ingresado" + idoperadora + " no corresponde al Carrier seleccionado. " + nombre;
                }
            }
        ...
        {
            //Aqui comento Pablo 08032018
            //e = new errRecarga { idtarjeta = idnuevatarjeta, PaymentID = paymentid, err = 4, errVs = 11, errRs = errRs, tresp = new tresp { rcode_description = "Ocurrio un error en la validación del número. Ex" } };
            resp = new Response3dSecure { err = true, desc = "Ocurrio un error en la validación del número. Ex" };
        }
        #endregion
        ...
        if (!resp.err)
        {
            ...
            try
            {
                ...
                if (rm == null && r != null)
                    p = LlenaPaquete(r.idpaquete);
                if (vez == 2)
                    t = LlenaTarjeta(t.idtarjeta);

                OpenpayAPI openpay = new OpenpayAPI(OPAYprivatekey, OPAYmerchantid);
                openpay.Production = false;
                Customer customer = new Customer();
                if (String.IsNullOrEmpty(u.customerid))
                {
                    customer.Name = u.first_name;
                    customer.LastName = u.last_name;
                    customer.PhoneNumber = u.numerocontacto;
                    customer.Email = u.email;
                    customer.RequiresAccount = false;
                    customer = openpay.CustomerService.Create(customer);
                    if (!String.IsNullOrEmpty(customer.Id))
                    {
                        string cs = ConfigurationManager.ConnectionStrings["ac"].ConnectionString;
                        using (var conn = new MySqlConnection(cs))
                        {
                            string command = "update usuario set customerid = @customerid where idusuario = @idusuario";
                            using (var cmd = new MySqlCommand(command, conn))
                            {
                                cmd.Parameters.Add("@customerid", MySqlDbType.VarChar, 50).Value = customer.Id;
                                cmd.Parameters.Add("@idusuario", MySqlDbType.Int32).Value = u.idusuario;
                                conn.Open();
                                cmd.ExecuteNonQuery();
                            }
                        }
                    ...
                    customer = openpay.CustomerService.Get(u.customerid);

                Card c = null;
                if (vez == 1)
                {
                    idnuevatarjeta = AltaTarjeta(t, deviceid, customer.Id, out c);
                    ExecuteSQL("update recarga set idtarjeta = " + idnuevatarjeta.ToString() + " where idrecarga = " + r.idrecarga.ToString());
                }
                if (vez == 2)
                {
                    //c = openpay.CardService.Get(t.permtoken);
                    ExecuteSQL("update recarga set idtarjeta = " + t.idtarjeta.ToString() + " where idrecarga = " + r.idrecarga.ToString());
                }
                if ((c != null && !String.IsNullOrEmpty(c.Id)) || (!String.IsNullOrEmpty(t.permtoken)))
                {
                    ChargeRequest request = new ChargeRequest();
                    request.Method = "card";
                    if (vez == 1)
                        request.SourceId = c.Id; //t.permtoken;
                    if (vez == 2)
                        request.SourceId = t.permtoken;
                    //request.Card = c;
                    request.Amount = p.monto;
                    request.Currency = "MXN";
                    request.Description = p.paquete;
                    request.OrderId = r.TransactionID;
                    if (deviceid != null)
                        request.DeviceSessionId = deviceid;
                    //request.Customer = customer;
                    request.Use3DSecure = true;
                    //request.RedirectUrl = "https://asicompras.com/confir_recarga.aspx";
                    request.RedirectUrl = "http://localhost/asicomprasweb/confir_recarga.aspx";

                    Charge charge = openpay.ChargeService.Create(customer.Id, request);
                    if (charge != null)
                    {
                        if (charge.Status == "charge_pending")
                        {
                            resp = new Response3dSecure { err = false, desc = charge.PaymentMethod.Url };
                        }
                    }
                }
            }
            ...
            {
                resp = new Response3dSecure { err = true, desc = ex.ErrorCode.ToString() };
            }
        }
        return JsonConvert.SerializeObject(resp);
    }

    public void PruebaOPENPAY(string idopenpaytr)
    {
        try
        {
            OpenpayAPI openpay = new OpenpayAPI(OPAYprivatekey, OPAYmerchantid);
            openpay.Production = false;
            var charge = openpay.ChargeService.Get(idopenpaytr);
        }
        catch (Exception ex)
        {
    ...
    public async Task<string> CargaOpenPay1aVez(Recarga r, RecargaMonedero rm, Tarjeta t, string idopenpaytr, string deviceid)
    {
        string riskxml = ""; string json = "";
        string errorvestadetallado = "";
        string responseString2 = "";
        tresp tresp = new tresp();
        int err = -1; int errVs = -1; int errRs = -1; string paymentid = "";
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    errVs = 1;
                    tresp.rcode_description = !String.IsNullOrEmpty(charge.ErrorMessage) ? charge.ErrorMessage.Replace("\"", "").Replace("'", "") : "";
                }
                err = 2;
                //errorvestadetallado = "Charge= AuthResultCode: " + sale.AuthResultCode + " AVSResultCode: " + sale.AVSResultCode + " CVNResultCode: " + sale.CVNResultCode + " PaymentStatus: " + sale.PaymentStatus + " ResponseCode: " + sale.ResponseCode;

                int numhoy = int.Parse(SelectCountSQL("SET time_zone = '-6:00'; select count(distinct r.idrecarga), date_format(u.fechaalta, '%Y%m%d'), date_format(now(), '%Y%m%d') from recarga r left join usuario u on r.idusuario = u.idusuario where r.idusuario = " + u.idusuario + " and r.idformapago = 2 and r.errVs = 0 and date_format(u.fechaalta, '%Y%m%d') = date_format(now(), '%Y%m%d')").ToString());
                if (numhoy == 3)
                    tresp.rcode_description = "En su primer dia solo se permite hacer 3 recargas. Si desea hacer mas, intente mañana.";
            }
            string stm = "";
            if (rm == null && r != null)
                stm = "update recarga set errvestadetallado = '" + tresp.rcode_description + "', err = " + err.ToString() + ", errVs = " + errVs.ToString() + ", errRs = " + errRs.ToString() + ", PaymentID = '" + paymentid + "' where idrecarga = " + r.idrecarga.ToString();
            else
            ...
            ExecuteSQL(stm);
            e = new errRecarga { idrecarga = r.idrecarga, idtarjeta = t.idtarjeta, desc = "", PaymentID = paymentid, err = err, errVs = errVs, errRs = errRs, tresp = tresp };
        }
        catch (Exception ex)
        {
        ...
        if ((idusr > 0 && email.Length == 0) || (idusr == 0 && email.Length > 0 && Regex.IsMatch(email, emailRegex, RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(250))))
        {
            ...
            using (var conn = new MySqlConnection(cs))
            {
                if (idusr > 0)
                    ...
                    //command = "select * from usuario where idusuario = @idusuario and bloqueado = @bloqueado limit 1;";
                else
                {
                    if (nip.Length > 0)
    ...
    public string GetUserV2(int idusr, string email, string nip)
    {
        int nummaxdeintentos = int.Parse(GetConfig(3).ToString());
        string SaltCryptoKey = ConfigurationManager.AppSettings["SaltCryptoKey"];
        string PwdCryptoKey = ConfigurationManager.AppSettings["PwdCryptoKey"];
    ...
    public string GetUsers()
    {
        ...
        using (var conn = new MySqlConnection(cs))
        {
            string command = "select u.idusuario, u.fechaalta, (select fecha from recarga where err = 0 and idusuario = u.idusuario order by fecha asc limit 1) as FechaPrimera, (select fecha from recarga where err = 0 and idusuario = u.idusuario order by fecha desc limit 1) as FechaUltima, (select count(idrecarga) from recarga where err = 0 and idusuario = u.idusuario) as NumRecargas, u.email, u.name, u.numerocontacto from usuario u order by u.fechaalta desc limit 500";
            using (var cmd = new MySqlCommand(command, conn))
            {
                MySqlDataAdapter adapt = new MySqlDataAdapter(cmd);
                conn.Open();
    ...
    { //semanal
        ...
        using (var conn = new MySqlConnection(cs))
        {
            switch (tipo)
            {
                case 1:
                    { //altas y ventas
                        command = "Select count(u.idusuario) as altas ,week(u.fechaalta)  as semana , year(u.fechaalta) as anio ,(select sum(p.monto) from recarga r inner join catpaquete p on p.idpaquete = r.idpaquete where week(r.fecha) = semana and r.err=0) ventas from usuario u where u.activo=1 group by anio,semana order by u.fechaalta asc limit 1000;";
                        break;
                    }
                case 2:
                    { //eventos
                        command = "Select count(r.idrecarga) as altas ,week(r.fecha)  as semana , year(r.fecha) as anio,count(r.idrecarga) as ventas  from recarga r group by anio,semana order by r.fecha asc limit 1000;";
                        break;
                    }
                case 3:
                    { //completo
                        command = "Select count(r.idrecarga) as altas ,week(r.fecha)  as semana , year(r.fecha) as anio,count(r.idrecarga) as ventas  from recarga r where err=0 and errVs=0 and errRs=0 group by anio,semana order by r.fecha asc limit 1000;";
                        break;
                    }
                case 4:
                    { //ticket prom
                        command = "select r.altas,r.semana,r.anio,FORMAT(r.res2/r.res1, 2) ventas from(Select count(r.idrecarga) as altas ,week(r.fecha)  as semana , year(r.fecha) as anio,count(r.idrecarga) as res1,sum(p.monto) res2 from recarga r inner join catpaquete p on p.idpaquete = r.idpaquete where err=0 and errVs=0 and errRs=0 group by anio,semana order by r.fecha asc limit 1000)r;";
                        break;
                    }

                case 5:
                    { //err vesta
                        command = "Select count(r.idrecarga) as altas ,week(r.fecha)  as semana , year(r.fecha) as anio,count(r.idrecarga) as ventas  from recarga r where errVs=1 group by anio,semana order by r.fecha asc limit 1000;";
                        break;
                    }
                case 6:
                    { //err banco
                        command = "Select count(r.idrecarga) as altas ,week(r.fecha)  as semana , year(r.fecha) as anio,count(r.idrecarga) as ventas  from recarga r where errVs=6 group by anio,semana order by r.fecha asc limit 1000;";
                        break;
                    }
            }
    ...
    public string GetMesDatos(int tipo)
    {
        ...
        using (var conn = new MySqlConnection(cs))
        {
            switch (tipo)
            {
                case 1:
                    { //altas 
                        command = " select monthname(u.fechaalta) as mes, year(u.fechaalta) as anio, count(u.idusuario) as num from usuario u   where u.activo = 1 group by anio, mes order by u.fechaalta asc limit 1000;";
                        break;
                    }
                case 2:
                    { //ventas
                        command = "select monthname(r.fecha) as mes, year(r.fecha) as anio , SUM(p.monto) as num from recarga r  inner join catpaquete p on p.idpaquete = r.idpaquete   where r.err = 0 group by anio,mes order by r.fecha asc limit 1000;";
                        break;
                    }
                case 3:
                    {
                        //eventos
                        command = "select monthname(r.fecha) as mes, year(r.fecha) as anio , count(r.idrecarga) as num from recarga r   group by anio,mes order by r.fecha asc limit 1000;";
                        break;
                    }
                case 4:
                    {
                        //completo
                        command = "select monthname(r.fecha) as mes, year(r.fecha) as anio , count(r.idrecarga) as num from recarga r where err=0 and errVs=0 and errRs=0 group by anio,mes order by r.fecha asc limit 1000;";
                        break;
                    }
                case 5:
                    {
                        //ticketprom
                        command = "select  mes, anio ,FORMAT(r.res2/r.res1, 2) as num from (select monthname(r.fecha) as mes, year(r.fecha) as anio , SUM(p.monto) as res2,  count(r.idrecarga) res1 from recarga r  inner join catpaquete p on p.idpaquete = r.idpaquete   where r.err = 0 and errVs=0 and errRs=0 group by anio,mes order by r.fecha asc limit 1000) r;";
                        break;
                    }
                case 6:
                    { //err vesta
                        command = "select monthname(r.fecha) as mes, year(r.fecha) as anio , count(r.idrecarga) as num from recarga r where errVs=1 group by anio,mes order by r.fecha asc limit 1000;";
                        break;
                    }
                case 7:
                    { //err banco
                        command = "select monthname(r.fecha) as mes, year(r.fecha) as anio , count(r.idrecarga) as num from recarga r where errVs=6 group by anio,mes order by r.fecha asc limit 1000;";
                        break;
                    }
            }
    ...
    public string GetDiaDatos(int tipo)
    {
        ...
        using (var conn = new MySqlConnection(cs))
        {
            switch (tipo)
            {
                case 1:
                    { //altas 
                        command = "select day(fechaalta)  dia,count(u.idusuario) num from usuario u where week(fechaalta)=week(CURDATE()) and activo=1 group by day(fechaalta) order by fechaalta asc limit 1000;";
                        break;
                    }
                case 2:
                    { //ventas
                        command = "select day(r.fecha) dia,sum(p.monto) num from recarga r inner join catpaquete p on p.idpaquete = r.idpaquete where week(r.fecha)=week(CURDATE()) and r.err=0 group by day(r.fecha) order by r.fecha asc  limit 1000;";
                        break;
                    }
                case 3:
                    {
                        //eventos
                        command = "select day(r.fecha) dia,count(r.idrecarga) num from recarga r  where week(r.fecha)=week(CURDATE())  group by day(r.fecha) order by r.fecha asc limit 1000;";
                        break;
                    }
                ...
                case 4:
                    {
                        //completo
                        command = "select day(r.fecha) dia,count(r.idrecarga) num from recarga r where err=0 and errVs=0 and errRs=0 and week(r.fecha)=week(CURDATE())  group by day(r.fecha)order by fecha asc limit 1000;";
                        break;
                    }
                case 5:
                    {
                        //ticket prom
                        command = "select day(r.fecha) dia, FORMAT(r.res2/r.res1, 2) num from (select r.fecha,count(r.idrecarga) res1, sum(p.monto) res2 from recarga r inner join catpaquete p on p.idpaquete = r.idpaquete where week(r.fecha)=week(CURDATE()) and err=0 and errVs=0 and errRs=0  group by day(r.fecha)order by fecha asc limit 1000) r;";
                        break;
                    }
                case 6:
                    { //err vesta
                        command = "select day(r.fecha) dia,count(r.idrecarga) num from recarga r  where week(r.fecha)=week(CURDATE()) and  errVs=1 group by day(r.fecha) order by r.fecha asc limit 1000;";
                        break;
                    }
                case 7:
                    { //err banco
                        command = "select day(r.fecha) dia,count(r.idrecarga) num from recarga r  where week(r.fecha)=week(CURDATE()) and  errVs=6 group by day(r.fecha) order by r.fecha asc limit 1000;";
                        break;
                    }
    ...
    public string GetMonDiario() {
        ...
        using (var conn = new MySqlConnection(cs))
        {
          command = "select day(fechaalta)  dia,count(u.idusuario) AS DATO, 'ALTA' from usuario u where day(fechaalta)=day(CURDATE()) and activo=1 group by day(fechaalta) limit 1000 UNION select day(r.fecha) dia,count(r.idrecarga)AS DATO, 'EVENTOS' from recarga r  where day(r.fecha)=day(CURDATE())  group by day(r.fecha) limit 1000 UNION select day(r.fecha) dia,count(r.idrecarga) AS DATO,'COMPLETO' from recarga r where err=0 and errVs=0 and errRs=0 and day(r.fecha)=day(CURDATE())  group by day(r.fecha) asc limit 1000 UNION select day(r.fecha) dia,count(r.idrecarga) AS DATO, 'VESTA' from recarga r  where day(r.fecha)=day(CURDATE()) and  errVs=1 group by day(r.fecha)  asc limit 1000 UNION select day(r.fecha) dia,count(r.idrecarga)AS DATO,'BANCO' from recarga r  where day(r.fecha)=day(CURDATE()) and  errVs=6 group by day(r.fecha)  asc limit 1000;";
    ...
    public string GetCarrier(int tipo, int operacion)
    {
        string SaltCryptoKey = ConfigurationManager.AppSettings["SaltCryptoKey"];
        string PwdCryptoKey = ConfigurationManager.AppSettings["PwdCryptoKey"];
        string json = "";
                    ...
                    { //importe
                        switch (operacion)
                        {
                            case 1:
                                {
                                    //diario
                                    command = "select day(r.fecha) as fech,o.operadora,p.tipo ,sum(p.monto) as vent, r.fecha as fech2   from recarga r  inner join catpaquete p on p.idpaquete = r.idpaquete    inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0 AND week(r.fecha)=week(CURDATE())     group by day(r.fecha),p.tipo,o.operadora            limit 1000  UNION   select day(r.fecha) as fech, 'MVNO', p.tipo, sum(p.monto) as vent,  r.fecha as fech2        from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1 and week(r.fecha)=week(CURDATE())      group by day(r.fecha), p.tipo       limit 1000  UNION   select day(r.fecha) as fech, 'PAQ', p.tipo, sum(p.monto) as vent, r.fecha as fech2      from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and tipo= 1 and week(r.fecha)=week(CURDATE())       group by day(r.fecha)       order by fech2 asc      limit 1000;";
                                    break;
                                }
                            case 2:
                                {
                                    //semanal
                                    command = "select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech ,o.operadora,p.tipo ,sum(p.monto) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0    group by year(r.fecha),week(r.fecha),p.tipo,o.operadora       limit 1000 UNION select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'MVNO', p.tipo, sum(p.monto) as vent , r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1    group by year(r.fecha),week(r.fecha), p.tipo    limit 1000 UNION select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'PAQ', p.tipo, sum(p.monto) as vent,  r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0  and tipo = 1    group by year(r.fecha),week(r.fecha)  order by fech2 asc    limit 1000; ";
                                    break;
                                }
                            case 3:
                                {
                                    //mensual
                                    command = "select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech ,o.operadora, p.tipo,sum(p.monto) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0    group by year(r.fecha),monthname(r.fecha),p.tipo,o.operadora       limit 1000 UNION select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'MVNO', p.tipo, sum(p.monto) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1    group by year(r.fecha),monthname(r.fecha), p.tipo    limit 1000 UNION select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'PAQ', p.tipo, sum(p.monto) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0  and tipo = 1    group by year(r.fecha),monthname(r.fecha)  order by fech2 asc    limit 1000;";
                                    break;
                                }
                        }
                        break;
                    ...
                    { //eventos
                        switch (operacion)
                        {
                            case 1:
                                {
                                    //diario
                                    command = "select day(r.fecha) as fech,o.operadora,p.tipo ,count(r.idrecarga) as vent, r.fecha as fech2     from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0 AND week(r.fecha)=week(CURDATE())     group by day(r.fecha),p.tipo,o.operadora            limit 1000  UNION   select day(r.fecha) as fech, 'MVNO', p.tipo, count(r.idrecarga) as vent, r.fecha as fech2       from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1 and week(r.fecha)=week(CURDATE())      group by day(r.fecha), p.tipo       limit 1000  UNION   select day(r.fecha) as fech, 'PAQ', p.tipo, count(r.idrecarga) as vent, r.fecha as fech2        from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and tipo= 1 and week(r.fecha)=week(CURDATE())       group by day(r.fecha)       order by fech2 asc      limit 1000;";
                                    break;
                                }
                            case 2:
                                {
                                    //semanal
                                    command = "select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech ,o.operadora,p.tipo ,count(r.idrecarga) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0    group by year(r.fecha),week(r.fecha),p.tipo,o.operadora       limit 1000 UNION select CATS(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'MVNO', p.tipo, count(r.idrecarga) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1    group by year(r.fecha),week(r.fecha), p.tipo    limit 1000 UNION select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'PAQ', p.tipo, count(r.idrecarga) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0  and tipo = 1    group by year(r.fecha),week(r.fecha)  order by fech2 asc    limit 1000; ";
                                    break;
                                }
                            case 3:
                                {
                                    //mensual
                                    command = "select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech ,o.operadora, p.tipo,count(r.idrecarga) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0    group by year(r.fecha),monthname(r.fecha),p.tipo,o.operadora       limit 1000 UNION select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'MVNO', p.tipo, count(r.idrecarga) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1    group by year(r.fecha),monthname(r.fecha), p.tipo    limit 1000 UNION select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'PAQ', p.tipo, count(r.idrecarga) as vent, r.fecha as fech2    from recarga r        inner join catpaquete p on p.idpaquete = r.idpaquete        inner join catoperadora o on p.idoperadora=o.idoperadora    where r.err = 0  and tipo = 1    group by year(r.fecha),monthname(r.fecha)  order by fech2 asc    limit 1000;";
                                    break;
                                }
                        }
                        break;
                ...
                case 3:
                    {
                        ...
                        switch (operacion)
                        {
                            case 1:
                                {
                                    //diario
                                    command = "select   fech ,operadora,tipo ,FORMAT(res2/res1, 2) vent,  fech2 from (    select day(r.fecha) as fech,o.operadora,p.tipo ,sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2      from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0 AND week(r.fecha)=week(CURDATE())     group by day(r.fecha),p.tipo,o.operadora            limit 1000  UNION   select day(r.fecha) as fech, 'MVNO', p.tipo, sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2       from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1 and week(r.fecha)=week(CURDATE())      group by day(r.fecha), p.tipo       limit 1000  UNION   select day(r.fecha) as fech, 'PAQ', p.tipo,sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2     from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and tipo= 1 and week(r.fecha)=week(CURDATE())       group by day(r.fecha)               limit 1000) r group by fech,operadora,tipo  order by fech2 asc;        ";
                                    break;
                                }
                            case 2:
                                {
                                    //semanal
                                    command = "select   fech ,operadora,tipo ,FORMAT(res2/res1, 2) vent,fech2 from (    select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech,o.operadora,p.tipo ,sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2     from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0       group by year(r.fecha),week(r.fecha),p.tipo,o.operadora         limit 1000  UNION   select CAST(concat(week(r.fecha),'-',year(r.fecha))  AS CHAR(10000) CHARACTER SET utf8) as fech, 'MVNO', p.tipo, sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2       from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1        group by year(r.fecha),week(r.fecha), p.tipo        limit 1000  UNION   select CAST(concat(week(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'PAQ', p.tipo,sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2      from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and tipo= 1         group by year(r.fecha),week(r.fecha)                limit 1000) r group by fech,operadora,tipo  order by fech2 asc;";
                                    break;
                                }
                            case 3:
                                {
                                    //mensual
                                    command = "select   fech ,operadora,tipo ,FORMAT(res2/res1, 2) vent,fech2 from (    select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech,o.operadora,p.tipo ,sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2        from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora in ('TELCEL', 'MOVISTAR', 'ATT') AND TIPO=0       group by year(r.fecha),monthname(r.fecha),p.tipo,o.operadora            limit 1000  UNION   select CAST(concat(monthname(r.fecha),'-',year(r.fecha)) AS CHAR(10000) CHARACTER SET utf8) as fech, 'MVNO', p.tipo, sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2       from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and operadora not in ('TELCEL', 'MOVISTAR', 'ATT') and tipo != 1        group by year(r.fecha),monthname(r.fecha), p.tipo       limit 1000  UNION   select CAST(concat(monthname(r.fecha),'-',year(r.fecha))  AS CHAR(10000) CHARACTER SET utf8) as fech, 'PAQ', p.tipo,sum(p.monto) res2, count(r.idrecarga) res1, r.fecha as fech2        from recarga r          inner join catpaquete p on p.idpaquete = r.idpaquete            inner join catoperadora o on p.idoperadora=o.idoperadora        where r.err = 0 and tipo= 1         group by year(r.fecha),monthname(r.fecha)               limit 1000) r group by fech,operadora,tipo  order by fech2 asc;";
                                    break;
                                }
                        }
                        break;
...
where r.fecha between str_to_date('" + fini + @"', '%Y%m%d') and str_to_date('" + ffin + @"', '%Y%m%d')");
    ...
    //-----------------------USUARIOS POR FECHA
    public byte[] ExportaRepUsrF(string fini, string ffin)
    {
        byte[] ret;
        DataTable dt = new DataTable();
        dt = JsonConvert.DeserializeObject<DataTable>(GetUsersF(fini, ffin));
        using (var exporter = new NpoiExport())
        {
            exporter.ExportDataTableToWorkbook(dt, "Results", "U");
            ret = exporter.GetBytes();
    ...
    public string GetUsersF(string fini, string ffin)
    {
        string SaltCryptoKey = ConfigurationManager.AppSettings["SaltCryptoKey"];
        string PwdCryptoKey = ConfigurationManager.AppSettings["PwdCryptoKey"];
        string json = "";
        ...
        using (var conn = new MySqlConnection(cs))
        {
            ...
            string command = "select u.idusuario, u.fechaalta, (select fecha from recarga where err = 0 and idusuario = u.idusuario order by fecha asc limit 1) as FechaPrimera, (select fecha from recarga where err = 0 and idusuario = u.idusuario order by fecha desc limit 1) as FechaUltima, (select count(idrecarga) from recarga where err = 0 and idusuario = u.idusuario) as NumRecargas, u.email, u.name, u.numerocontacto from usuario u where u.fechaalta between str_to_date('" + fini + "', '%Y%m%d') and str_to_date('" + ffin + "', '%Y%m%d') order by u.fechaalta desc limit 500";
            using (var cmd = new MySqlCommand(command, conn))
            {
                MySqlDataAdapter adapt = new MySqlDataAdapter(cmd);
                conn.Open();
    ...
    private void EnviaMail(string email, char tipo, string liga, Usuario u = null, byte[] reporte = null, Recarga r = null, catPaquete cp = null)
    {
        ...
        if (tipo == 'C') //Enviar confirmacion de regalo
        {
            SUBJECT = "Se le abono en monedero ";
            readText = "Gracias por su preferencia!! Su regalo de saldo a monedero ha sido abonado, continue comprando con </br></br>¡Saludos!";
        }

        String BODY = readText;
        const String SMTP_USERNAME = "contacto@asicompras.com";
        const String SMTP_PASSWORD = "KsTRndeoPW.17";
